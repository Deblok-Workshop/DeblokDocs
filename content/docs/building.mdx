...
```bash
sudo bash run.sh debian-base # use port 3000
sudo bash run.sh debian-base 59000 # use port 59000
```
...
You can use this to automatically tag it for NewDeblok.
```bash
sudo docker build -t ghcr.io/deblok-workshop/deblokdocker/debian-base:latest ./debian-base
```
</Step>
</Steps>